function [rho_A, rho_B, rho_C, wave] = density_BCC_SISO(filename)
...
f_A = f(1); f_B = f(2); f_C = f(3);
const = f_C/N_particles; sigma_smear = 0.1;     % const = f/(No. of particles)
Rsph = (3*f_C/(4*pi*N_particles))^(1/3);                    % Sphere radius (Mass cons.) 
...
for i=0:grid(1)/2
    for j=0:grid(2)-1  
        for k=0:grid(3)-1
            ...
            elseif(mod(sum(G),2)==0)
                t = t+1;
                wave(t,:) = G;
                qR = (sum(G.*G))^(0.5)*2*pi*Rsph;
                rho_C(t,1) = const*2*form_factor(qR)*exp(-sigma_smear*qR^2/2);
                rho_A(t,1) = -(f_A/(f_A + f_B))*rho_C(t);
                rho_B(t,1) = -(f_B/(f_A + f_B))*rho_C(t);
