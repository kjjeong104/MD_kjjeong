function terpolymer_visual(filename)

C = textread(filename, '%s','delimiter', '\n');
dim = str2num(C{3});                              % Read dimension
cell = str2num(C{9});                             % Read cell parameters
...
% Volume fraction of C monomer type. 
data = smooth3(c,'box',5);          % Smooth out the volume fraction
isovalue =0.5;                      
...
set(p2,'AmbientStrength',.8); 
set(p1,'AmbientStrength',.7); 
