function [rho_A, rho_B, wave] = density_BCC(filename)
...
const = f(1)/N_particles; sigma_smear = 0.28;				% const = f/(No. of particles)
Rsph = (3*f(1)/(4*pi*N_particles))^(1/3);                   % Particle size
    ...
    for j=0:grid(2)-1  
        for k=0:grid(3)-1
          ...
          elseif(mod(sum(G),2)==0)
             ...
             rho_A(t,1) = const*2*form_factor(qR)*exp(-(sigma_smear)^2*qR^2/2);
             rho_B(t,1) = -rho_A(t);
            
          % Waves not satisfying reflection conditions have rho = 0. 
          else
              t = t+1;
              wave(t,:)  = G;
              rho_A(t,1) = 0;
