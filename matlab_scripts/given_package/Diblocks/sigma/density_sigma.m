function [rho_A, rho_B, wave] = density_sigma(filename)
...
V_cell = cell_param(1)^2*cell_param(2);
const = f(1)/N_particles; sigma_smear = 0.35;     % const = f/(No. of particles)
Rsph = cell_param(1)/10;
%Rsph =  (3*f(1)*V_cell/(4*pi*N_particles))^(1/3); % Particle size 
    ...
    for j=0:grid(2)-1  
        for k=0:grid(3)-1
          ...
          if(G(1)==0 && G(2)==0 && G(3)==0)
             ...
             rho_A(t,1) = f(1); 		  
             rho_B(t,1) = f(2);
			% R and I are the real and imaginary part of  
			% volume fraction, respectively. Since volume fraction is real, 
            % I is approximately 0, and so it is not used in the calculation.
            ...
            [R,I] = sum_ff(G,pos);
            G_dot_R = [G(1) G(2) G(3)].*...
                        [1/cell_param(1) 1/cell_param(1) 1/cell_param(2)];
            qR = (sum(G_dot_R.*G_dot_R))^(0.5)*2*pi*Rsph;
...
% The function sum_ff to calculate sum of form factors weighted by exp(q.r)
