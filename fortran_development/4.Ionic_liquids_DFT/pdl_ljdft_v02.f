...
C       v01 : apply LJ attraction. for PSILA:used 1-body pot
C       introduce(1): LJ att potential by electrode surface
C       introduce(2): LJ att correlation functions.
C       note:EPP,EPM,EPS:LJ epsilon for cation,anion,surface
C       estar:related to reduced coulomb coupl. param.
C       (see Hansen&McDonald ch10. p417)
...
1191    FORMAT(2X,'DIAMETER RATIO(ANION)',4X,E18.8/
...
C       Calculate LJ attraction pot(\beta*\psi_ljatt(z))
        ...
        CALL COUL(DENP,DENM,PSI)
        CALL VDWPOT(DENP,DENM,PSILA)
C
C        CALCULATE C2ELINT
         CALL C2ELINT(DENP,DENM,DENAVP,DENAVM,DENBARP,DENBARM,
     C               C1EL11,C1EL12,C1EL21,C1EL22)
C        Calculate C2LAINT (LJ attraction)
C         CALL C2LAINT(DENP,DENM,DENAVP,DENAVM,DENBARP,DENBARM,
C     C               C1LA11,C1LA12,C1LA21,C1LA22)
         DO I = 1, MESH
          C1EL1P(I) = C1EL11(I) - C1EL12(I)
          C1EL1M(I) = C1EL21(I) - C1EL22(I)
C          C1LA1P(I) = C1LA11(I) - C1LA12(I)
C          C1LA1M(I) = C1LA21(I) - C1LA22(I)
         ENDDO
C       DEFINE \rho_0
...
C          FM=-QM*PSI(I)+C1MB-C1MA+C1EL21(I)+C1EL22(I)
          FP=-QP*PSI(I)-PSILA(I)+C1PB-C1PA+C1EL11(I)+C1EL12(I)
C     C       +C1LA11(I)+C1LA12(I)
          FM=-QM*PSI(I)-PSILA(I)+C1MB-C1MA+C1EL21(I)+C1EL22(I)
...
C
        ...
        IF(MOD(NITER,50).EQ.0)THEN
         ...
         WRITE(9,121)NITER,SUMP,SUMM,SUMT,telapsed-tstart
         WRITE(6,121)NITER,SUMP,SUMM,SUMT,telapsed-tstart
        ENDIF
...
C     C              DENBARM(I),C1EL1P(I),C1EL1M(I)
        WRITE(1,122)RI,DENPI,DENMI,PSI(I)
C        WRITE(9,122)RI,DENPI,DENMI,PSI,DENP(I),DENM(I),DENBARP(I),
C     C              DENBARM(I),C1EL1P(I),C1EL1M(I)
       ENDDO
...
C       without consideration of liquid layers
        SUBROUTINE VDWPOT(DENP,DENM,PSILA)
        IMPLICIT DOUBLE PRECISION(A-H,O-Z)
        PARAMETER(PI = 3.141592653589793D0, FPI = 4.0D0*PI)
        PARAMETER(ALH = 30.0D0, DZ = 0.02D0, NMAX=1+(ALH/DZ))
        DIMENSION R(NMAX),DENP(NMAX),DENM(NMAX)
...
C       AP1,AM1,BP1,BM1 here are not integrands.
        DO I=2,MESH
         R3=R(I)**3
         R9=R3**R3**R3
         AP1=EP13*DENP(I)*S133*(S139/R9)/15.0D0
         AM1=EP23*DENM(I)*S233*(S239/R9)/15.0D0
         BP1=EP13*DENP(I)*S133*(S133/R3)/2.0D0
         BM1=EP23*DENM(I)*S233*(S233/R3)/2.0D0
         VALA(I)=AP1+AM1
         VALB(I)=BP1+BM1
        ENDDO
        VALA(1)=0.0D0
        ...
        DO I=1,MESH
         PSILA(I) = 0.0D0
          PSILA(I)=FPI*(VALA(I)-VALB(I))/3.0D0
        ENDDO
C
        RETURN
        END
...
C
     ...
     C                    C1LA11,C1LA12,C1LA21,C1LA22)
       ...
       DIMENSION R(NMAX),DENP(NMAX),DENM(NMAX)
       DIMENSION DENBARP(NMAX),DENBARM(NMAX)
       DIMENSION C2LA11(NMAX),C2LA12(NMAX),C2EL21(NMAX),C2EL22(NMAX)
       DIMENSION C1El11(NMAX),C1EL12(NMAX),C1EL21(NMAX),C1EL22(NMAX)
C
       COMMON/DIST/R
       COMMON/LJ/EPP,EPM,EPS
