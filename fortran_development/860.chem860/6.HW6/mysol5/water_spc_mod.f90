  MODULE water_spc_mod
      ...
      integer                                   :: move_option
      real(kind=8), allocatable, dimension(:,:)  :: x,xoxy,xm,xx,v,vi,f,fewald,frwald
...
!   read in the data file
    ...
    read(15,*)angmax    ! max mc angle change
...
!   need to calculate long range correction to 6-12
    ri3 = 1.d0/rcut**3
    e_lrc = 2*pi*(nmol**2)*(c12(1,1)*ri3/3.0D0-c06(1,1))*ri3/3.0D0/volume   ! some function of r13 nmol c12 and c06  
    write(*,'(a,f10.5)')' e_lrc = ',e_lrc/e_conv
...
!   ----------------------------------------------------------------
	    allocate(x(npart,3),v(npart,3),vi(npart,3),xm(npart,3),xx(npart,3),xoxy(nmol,3))
...
!    *******************************************************************
  ...
  subroutine rotate
    implicit none
    integer                          :: i,j,l
    real(kind=8)                     :: rnd,z,sthe,psi,cpsi,spsi,phi,cphi,sphi,the
    real(kind=8), dimension(3,3)     :: a
    real(kind=8), dimension(3)       :: zcm !center of mass
     
  
    call random_number(rnd)        !pick angle to be moved
    phi = (-1.d0+2.d0*rnd)*angmax
    call random_number(rnd)        !pick angle to be moved
    psi = (-1.d0+2.d0*rnd)*angmax
    call random_number(rnd)        !pick angle to be moved
	the = (-1.d0+2.d0*rnd)*angmax
	z = dcos(the)
!    z = -1.d0+2.d0*rnd
    cphi = dcos(phi)
    sphi = dsin(phi)
...
! Rotation matrix of Euler Angles, Z_1*X_2*Y_3. ref http://en.wikipedia.org/wiki/Euler_angles    
  ...
  subroutine rotate_trans
    implicit none
    integer                          :: i,j,l
	real(kind=8)		::	rnd,the,ax,ay,az,norm,cthe,sthe
	real(kind=8), dimension(3,3)	::	a
	real(kind=8), dimension(3)	::	zcm
  real(kind=8) ::  phi,psi
  ...
  psi = rnd*pi
  az = dcos(psi)
  ax = dcos(phi)*dsin(psi)
  ay = dsin(phi)*dsin(psi)
  
	call random_number(rnd) !pick angle to be moved
	the = (-1.d0+2.d0*rnd)*angmax
	cthe = dcos(the)
	sthe = dsin(the)
!rotational matrix : rotate by angle phi, with respect to axis (ax,ay,az)
!ref http://en.wikipedia.org/wiki/Rotation_matrix
	a(1,1)= cthe + ax**2*(1-cthe) 
	a(1,2)= ax*ay*(1-cthe)-az*sthe
	a(1,3)= ax*az*(1-cthe)+ay*sthe

	a(2,1)= ax*ay*(1-cthe)+az*sthe
	a(2,2)= cthe+ay**2*(1-cthe)
	a(2,3)= ay*az*(1-cthe)-ax*sthe

	a(3,1)= ax*az*(1-cthe)-ay*sthe
	a(3,2)= ay*az*(1-cthe)+ax*sthe
	a(3,3)= cthe+az**2*(1-cthe)
 ...
 !   make one of three kinds of moves
     ...
     case(2)
       ...
       do j = 1,natom
          ...
          x(natom*(o-1)+j,:) = new_mol(:,j)                  !update x vector position
       enddo 
  !rotate to random orientation. rotate by random angle with respect to random vector as a pivot.
...
!    calculate the 3 contributions to the energies
     ...
     enn = pot_sr+pot_lr-vself+pot_lj+e_lrc         ! new potential
...
!-----------------------------------------------------------------------------------
  ...
  subroutine mcvol(vpot,jstep) !attempts to displace a particle
    ...
    real(kind=8), dimension(3)       :: zcm,dzcm  !com and change in com
    real(kind=8)                     :: vpot,eno,enn,vo,lnvn,vn,boxo,boxn,arg,rco,kappao
...
!    ri3 = 8.d0/vo
    ri3 = 1.d0/(rco)**3
    e_lrc = 2*pi*(nmol)**2*(c12(1,1)*ri3/3.0D0-c06(1,1))*ri3/3.0D0/vo  !this needs to be fixed
!    print*, e_lrc/vo
...
!    write(*,*)' the old box length is ', boxo
!    write(*,*)' the new box length is ', boxn
    do i = 1,nmol
      do j = 1,natom
...
!    enn = pot_lr+pot_sr+pot_lj-vself    !why e_lrc is missed??????
    ri3n = 1.d0/(rcut)**3 ! I added
    e_lrcn = 2*pi*(nmol)**2*(c12(1,1)*ri3n/3.0D0-c06(1,1))*ri3n/3.0D0/vn ! I added
