...
!   need to calculate long range correction to 6-12
    ri3 = 1.d0/rcut**3
    e_lrc = 2*pi*(nmol/volume)*(c12(1,1)*ri3/3.0D0-c06(1,1))*ri3/3.0D0   ! some function of r13 nmol c12 and c06  
...
! Rotation matrix of Euler Angles, Z_1*X_2*Y_3. ref http://en.wikipedia.org/wiki/Euler_angles    
  ...
  subroutine rotate_trans
	...
	call random_number(rnd) !pick x component of rotational axis
	ax = rnd
        call random_number(rnd) !pick y component of rot axis
	ay = rnd
        call random_number(rnd) !pick z component of rot axis
	az = rnd
	norm = dsqrt(ax**2+ay**2+az**2)	!normalize the rot axis vector
	ax = ax / norm
	ay = ay / norm
	az = az / norm
	call random_number(rnd) !pick angle to be moved
...
!    ri3 = 8.d0/vo
    ...
    e_lrc = 2*pi*(nmol/vo)*(c12(1,1)*ri3/3.0D0-c06(1,1))*ri3/3.0D0  !this needs to be fixed
!    print*, e_lrc/vo
...
!    enn = pot_lr+pot_sr+pot_lj-vself    !why e_lrc is missed??????
    ri3n = 1.d0/(boxn/2.0D0)**3 ! I added
    e_lrcn = 2*pi*(nmol/vn)**2*(c12(1,1)*ri3n/3.0D0-c06(1,1))*ri3n/3.0D0 ! I added
