...
!   need to calculate long range correction to 6-12
    ri3 = 1.d0/rcut**3
    e_lrc = 0.0 ! some function of r13 nmol c12 and c06  
...
!-----------------------------------------------------------------------------------
  ...
  subroutine mcvol(vpot,jstep) !attempts to displace a particle
    ...
    real(kind=8)                     :: pot_lj_old,pot_sr_old,pot_lr_old,ri3
 !  --------------------------------------
 !  save the old values
    xm = x 
    pot_lj_old = pot_lj
...
!    ---------------------------------------------------------------------------
    ri3 = 8.d0/vo
    e_lrc = 0.d0   !this needs to be fixed
    if(e_lrc==0)write(*,*)' fix the above expression '
    eno = pot_lr+pot_sr+pot_lj-vself+e_lrc/vo
!   ------------------------------------------
