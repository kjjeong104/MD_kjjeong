...
! check that the value of kappa in ewald sum is OK.
  ...
  if(kappa_check>1.d-5)then
   write(16,'(a,d12.6)')'2.d0*erfc(0.5d0*kappa*box)/box =',kappa_check
...
! ----------------------------------------------------------------
  ...
  write(6,*)' density(g/cm^3)  pot_lr   pot_sr       pot_lj      vpot'
! ----------------------------------------------------------------
...
!      --------------------------------------------------------------
       write(6,'(6f12.5)')dens_cgs,pot_lr/e_conv,pot_sr/e_conv, &
