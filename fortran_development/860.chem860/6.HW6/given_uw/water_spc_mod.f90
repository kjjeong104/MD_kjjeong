  MODULE water_spc_mod
   ...
   !     subroutine potential_fast(o,del_eno)   !same as potential but used in mcmove
   !     subroutine self                        !calculate intramolecular coulomb interactions
   !     SUBROUTINE ewald_setup                 !set up the variables used in ewald
   !     subroutine kwald                       !long range k-space part of ewald
   !     subroutine rwald                       !short range r-space part of ewald
   !     subroutine kwald_fast                  !used in mcmove
   !     SUBROUTINE RWALD_fast (o,del_pot_sr)   !used in mcmove
   !     subroutine mcmove(vpot,istep)          !attempts to displace a particle
   !     subroutine mcvol(vpot,jstep)           !attempts to change the volume
   !     subroutine rot_trans(zcm,dzcm)         !three subroutines for moves
   !     subroutine rotate_trans(zcm,dzcm)
   !     subroutine rotate(zcm,dzcm)
   !     subroutine com(zcm)                    !find center of mass for a molecule
   !     subroutine rotation_matrix(dc,ds,rot_axis,rot_mat)  !set up rotation matrix for rotation about x, y, and z.
   !     subroutine center                      !keep particles in box
   !     subroutine masses                      !sets up masses given atomic numbers.
   !     REAL(kind=8)  FUNCTION erfc(x)
      ...
      character(len=4), dimension(50)           :: atom_name 
...
!   read in the data file
    ...
    do i = 1,natom
     ...
     amass(i) = mass(ni(i))
     write(ntape,'(a,f10.3)')' amass(i) = ',amass(i) 
    enddo
    cmass = sum(amass)
    ...
    read(15,*)delx      ! max mc com step size
    read(15,*)angmax    ! max mc angle step size
    ...
    read(15,*)ntape     ! =6 output to screen; =16 output to water_spc.dat
    close (15)
    if(ntape/=6.and.ntape/=16)then
       write(*,*)'check the value of ntape it must be 6 or 16'
       stop
    endif
!   --------------t--------------------------------------------------
...
!   ----------------------------------------------------------------
    ...
    beta = 1.d0/tstar
    rcut = 4.8d0                               !fixed value
    if(rcut>0.5d0*box)then
       write(*,*)'box is too small for rcut =',rcut
       write(*,*)'box/2 = ',box/2
       stop
    endif
!   ----------------------------------------------------------------
!   need to calculate long range correction to 6-12
    ri3 = 1.d0/rcut**3
    e_lrc = 0.d0  !Not really.  You need to add something for HW.
    write(ntape,'(a,f10.5)')' e_lrc = ',e_lrc/e_conv/volume
!   ----------------------------------------------------------------
    kappa = nkappa/box
    rc2 = rcut*rcut
    write(ntape,'(a,f8.4)')' the following are in A ps amu units'
    write(ntape,'(a,f8.4)')' the desired temp is    ',tstar
    write(ntape,'(a,i5)')' the move_option is    ',move_option
    write(ntape,'(a,f8.3)')' the box length is      ',box
    write(ntape,'(a,i8)')  ' number of particles is ',npart
    write(ntape,'(a,f8.5)')' the density is         ',density
    write(ntape,'(a,f8.5)')' the pressure is        ',pressure
    write(ntape,'(a,f8.5)')' the value of beta  is  ',beta
    write(ntape,'(a,f8.3)')' the volume is          ',volume
    write(ntape,'(a,f8.3)')' r cutoff is            ',rcut
    write(ntape,'(a,f8.3)')' molar mass is          ',cmass
!   ----------------------------------------------------------------
    allocate(x(npart,3),xm(npart,3))
    allocate(z(npart))
! ----------------------------------------------------------------
!   move atoms to center of mass frame
    write(ntape,*)' atoms in center of mass frame '
    do i = 1,3
     zcm = sum(xatom(i,:)*amass(:))/cmass
     xatom(i,:) = xatom(i,:)-zcm
    enddo
    write(ntape,'(3f10.5)')xatom
! ----------------------------------------------------------------
...
! ----------------------------------------------------------------
 ...
 if(iread==1)then
   ...
   read(18,*) box
   write(ntape,'(a,f10.4)')' the current box length is ',box
   volume = box**3
   read(18,*)x
   close(18)
 else                   ! pick points on a  cubic grid
  ...
  if(nmol==2)then ! just two molecules
     ...
     do iat = 1,natom
       nt = nt + 1
       x(nt,1) = 2*delta + xatom(1,iat)
       x(nt,2) = 2*delta + xatom(2,iat)
       x(nt,3) = 2*delta + xatom(3,iat)
     enddo
  else
   !put on a grid
    do i = 0,ipart-1
  ...
  if(nt/=npart)then
     write(ntape,*)' stopping since nt is not equal to npart '
     write(ntape,*)' nt    = ',nt
     write(ntape,*)' npart = ',npart
     stop
  endif
 endif
! ------------------------------------------------------
  write(ntape,*)' here are the original charges'
  write(ntape,*)qi
  qi = qi*coulomb*dsqrt(0.1d0*av*kc*1.d10)    !convert to ps A amu units
  write(ntape,*)' here are the scaled charges'
  write(ntape,*)qi
  k = 0
  do i = 1,nmol
   do j = 1,natom
   ...
   enddo  
  enddo
  ecut = (c12(1,1)/rc2**6-c06(1,1))/rc2**3  !cutoff value for the 6-12 potential
  ecut = 0
  write(ntape,*)' ecut = ',ecut
  end subroutine init
...
!-----------------------------------------------------------------------------------
  ...
  enddo
  vself = vself*nmol
  write(ntape,*)' the self energy is ',vself/e_conv
  end subroutine self
...
!    ** CALCULATE THE TOTAL K-SPACE POTENTIAL **
  ...
  subroutine mcmove(vpot,istep) !attempts to displace a particle
    ...
    real(kind=8)                     :: vpot,eno,enn,rnd,vr
 ...
 !   ------------------------------------------------------------------
 !   make one of two kinds of moves
     select case(move_option)
     case(1)
       call rotate(zcm,dzcm)   !rotate to random orientation
     case(2)
       call rotate_trans(zcm,dzcm)   !rotate to random orientation
     case(3)
       call rot_trans(zcm,dzcm)   !rotate to random orientation
     end select 
!    ---------------------------------------------------------------
     do j = 1,natom
...
! --------------------------------------------------------------------------------
  subroutine mcvol(vpot,jstep) !attempts to change the volume 
    ...
    real(kind=8)                     :: pot_lj_old,pot_sr_old,pot_lr_old
 !  --------------------------------------
 !  save the old values
    xm = x 
    pot_lj_old = pot_lj
    pot_lr_old = pot_lr
    pot_sr_old = pot_sr
    eno = pot_lr+pot_sr+pot_lj-vself
    boxo = box
    vo = box**3
!   ------------------------------------------
    call random_number(rnd)           !pick a random number for volume move
  ! lnvn=dlog(vo)+(rnd-0.5d0)*vmax    !currently set up to move the log(volume)
  ! vn = dexp(lnvn)
  ! vn = dexp(lnvn)
    vn= vo + (rnd-0.5d0)*vmax
...
!   calculate energies at new position
    ...
    enn = pot_lr+pot_sr+pot_lj-vself
!   Maybe a correction is needed here   
!   ------------------------------------------
!   decide if you want to take a move
    arg = -beta*((enn-eno) + pressure*(vn-vo)-(nmol)*dlog(vn/vo)/beta)
...
! --------------------------------------------------------------------------------
  subroutine rot_trans(zcm,dzcm)
    implicit none
    real(kind=8), dimension(3)       :: zcm,dzcm !com and change in com
    write(*,*)' This option does not exit.  You must write it.'
    stop
...
! --------------------------------------------------------------------------------
  subroutine rotate_trans(zcm,dzcm)
    implicit none
    real(kind=8), dimension(3)       :: zcm,dzcm !com and change in com
    write(*,*)' This option does not exist.  You must write it.'
    stop
...
! --------------------------------------------------------------------------------
  subroutine rotate(zcm,dzcm)
    ...
    real(kind=8), dimension(3)       :: zcm,dzcm !center of mass
    ...
    do j = 1,natom
       new_mol(:,j) = xtemp(:,j) + zcm + dzcm       !new position of atom
    enddo
...
! --------------------------------------------------------------------------------
  ...
  subroutine rotation_matrix(dc,ds,rot_axis,rot_mat)
  implicit none
  real(kind=8), dimension(3,3)     :: rot_mat
  integer      :: i,j,k,rot_axis
  real(kind=8) :: dc,ds
   i = mod(rot_axis-1,3)+1
   j = mod(rot_axis  ,3)+1
   k = mod(rot_axis+1,3)+1
   rot_mat = 0.d0
   rot_mat(i,i)= 1.0
   rot_mat(j,j)= dc
   rot_mat(k,k)= dc
