...
!   -----------------------
  subroutine input(irun,nequil)
    implicit none
    integer      :: irun,nequil
    open(15,file='lj_bias_chain.inp')
!   ----------------------------------------------------------------
...
! ----------------------------------------------------------------
  subroutine mcgrow(vpot,istep) !attempts to displace a particle
    implicit none
    integer                          :: i,j,o,omon,istep
    real(kind=8)                     :: vpot,eno,enn,eno_lj,eno_str,enn_lj,enn_str,rnd
    real(kind=8)                     :: wo, wn

    call random_number(rnd)        !pick molecule to be removed and randomly grown
    o=int(rnd*nmol)+1
    call random_number(rnd)        !pick the index of monomer to start growth
    omon=int(rnd*natom)+1
    do i=1,omon-1
     xn(i) = x(i)	!store positions that are not grown
    enddo 
