...
! -----------------------------------------------------------------------------------------------------
  ...
  write(*,*)'  time    vpot/N        etot        temp        pressure   '
  do i = 1,nrun
   call force(vpot)
   call integrate(etot,vpot,sumv,temp)   !integrate equations of motion
   if(mod(i,nsamp)==1)samps = samps + 1
   if(mod(i,nsamp)==1)call center
   if(mod(i,nsamp)==1)call pressure(temp,pideal,preal,virgsum)
   if(mod(i,nsamp)==1)write(*,'(5f14.8)')dt*(i-1),vpot/npart,etot,temp,preal
  enddo
  write(*,'(a,f10.5)')' The average pressure is ',density/beta+virgsum/(samps*3*box(1)*box(2)*box(3))
! ----------------------------------------------------------------
...
!-----------------------------------------------------------------------------------
  subroutine pressure(temp,pideal,preal,virgsum)
  use lj_mod
  implicit none
  integer	::	i,j	!counters
  real(kind=8)	::	pideal,preal,virgsum,vir,virsum,temp 
 !ideal pressure, real pressure,virial grand sum over the whole simulation,virial,virial sum in a single step
  real(kind=8), dimension(3)	:: dr !vector btwn atoms i and j
  real(kind=8)	::	r2,r2i,r6i,ff
  pideal = density*temp
  virsum = 0
  f = 0
  do i=1,npart-1
 	do j=i+1,npart
		...
		if(r2<=rc2)then
			...
			f(i,:) = f(i,:) + ff*dr
			vir = dot_product(dr,f(i,:))
			virsum = virsum + vir
		endif
	enddo
  enddo
  virgsum = virgsum + virsum
  preal = pideal + virsum / (3.0d0*box(1)*box(2)*box(3))  
