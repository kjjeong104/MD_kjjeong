...
! ---------------------------------------------------------------SECTION G  (binning has been added)
  ...
  do i = 1,ncycle
   ...
   call integrate(npart,dt,f,x,xx,xm,v ,etot,vpot,temp)   !integrate equations of motion
   tavg = tavg+temp 
   if(mod(i,nsamp)==1)call speedbin(npart,nbins,x,v,rbin,bn,temp,umatrix,root,iflag)
 ...
 !  if(mod(i,nsamp)==1)call nm_time(npart,req,x,v,q,pq,i,dt,root,bk,enm,umatrix,istart,etot)
  ...
  iflag = 1
  call speedbin(npart,nbins,x,v,rbin,bn,temp,umatrix,root,iflag)
...
!---------------------------------------------------------------------------
  subroutine speedbin(npart,nbins,x,v,rbin,bn,temp,umatrix,root,iflag)
  implicit none
