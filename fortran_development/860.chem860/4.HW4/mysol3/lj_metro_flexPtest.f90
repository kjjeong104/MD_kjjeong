...
!  --------------------------------------------------------------------------
  ...
  real(kind=8)               :: dens_avg,vavg,pavg            !average calculated density and potential,pressure
  real(kind=8)               :: rnd                      !random number
  real(kind=8)                                  :: vpot,PLJ, pres1
  real(kind=8)                                  :: dens_std,pstd !standard deviation of the blocks
  real(kind=8), allocatable, dimension(:)       :: dens,pres_block     !average density of a block 
! ----------------------------------------------------------------
...
!      call pressure(pres)        !
     ...
     if(mod(i,nsamp)==1)then
       ...
       call pressure(pres)
       write(6,'(5f14.8)')vpot,density,pres,box !PLJ(2,density)
     endif
  enddo
...
!      call pressure(pres)	!
     ...
     if(mod(i,nsamp)==1)then
       ...
       call potential_full(vpot)    !calculate the potential energy
       call pressure(pres)
       write(6,'(5f14.8)')vpot,density,pres,box
     endif
     vavg = vavg + vpot
  ...
  write(*,*)' Long range corrections of the energy are not included!!! '
  write(*,*)' You should include them.'
...
!-----------------------------------------------------------------------------------
  ...
  subroutine pressure(p)  !p is the pressure
  use lj_metro_npt_mod
  implicit none
  real(kind=8), dimension(3) :: dr
  real(kind=8)               :: r2,r6i,p,rc3it,rc9it
  integer                    :: i,j    !counters
  p = 0
  do i = 1,npart-1
   do j = i+1,npart
  ...
  p = (npart*tstar+16*p+32*pi*density*dble(npart)*(rc9it-rc3it)/3.0D0)/volume		!P LRC
