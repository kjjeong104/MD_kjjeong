...
! declare all the variables that will be used.
  implicit none
  integer, parameter                 :: npart=20,nint=19  !number of atoms and number internal dof
  integer                            :: ncycle,nsamp,istart=0
  integer                            :: i,j             !counter
  real*8                             :: fii,req,delt,dt,etot,time_tot,vpot,pot4
...
! ---------------------------------------------------------------SECTION G
  open(99,file='r4energy.dat')
  do i = 1,ncycle
   call force(npart,nint,x,f,r,fr,drx,fii,vpot,req,pot4) 
   ...
   if(mod(i,ncycle/40)==1)call jmol(npart,x)    !make a movie
	if(mod(i,10)==1) write(99,'(2f12.7)') i*dt,pot4
  enddo
...
!  ------------------------------------------------------------------------------
  ...
  pot4 = 0.5d0*fii*r(4)*r(4)	!calculate r(4) int.coord pot energy
