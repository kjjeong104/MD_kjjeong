  MODULE lj_ewald_mod
      ...
      integer, parameter             :: MAXK = 1000000 
...
!                    if(ksq==ksqmax-1) write(*,'(a,3i4,f12.7)')'ksq==ksqmax-1:(kx,ky,kz,kvec= )',kx,ky,kz,kvec(totk)
                 ENDIF
              enddo
           enddo
        enddo
        ...
        DO KX =  0, KMAX
           DO KY =  -KMAX, KMAX
              ...
              DO KZ =  -KMAX, KMAX
                 ...
                 IF ( ( KSQ < KSQMAX ) .AND. ( KSQ /= 0 ) ) THEN
                    TOTK = TOTK + 1
                    if(ksq==ksqmax-1) write(*,'(a,3i4)')'ksq==ksqmax-1:(kx,ky,kz= )',kx,ky,kz
