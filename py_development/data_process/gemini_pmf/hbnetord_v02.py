...
# v01 : total #Hbond , v02 : can separately count acception and donation
...
def main():
  ...
  maxnl=8 #max number of layers we inspect
  flag,nl=0,1
  for line in accfile:
  ...
  usedw=[] #list of used water and solute molecules for hbond
    ...
    for row in sets:
      if row[1] in recent: #if the recent layer element is acceptor
        if row[0] not in usedw:
          ...
          usedw.append(row[0])
      elif row[0] in recent: #if the recent layer element is donor
        if row[1] not in usedw:
          onelayer.append(row[1])
          usedw.append(row[1])
  ...
  nhac=numpy.zeros(maxnl+1,int)
  nhdo=numpy.zeros(maxnl+1,int)
 ...
 #printing section
  outfile.write('layer# mem# tothbn avehbn aveacc avedon\n')
  for i in range(maxnl+1):
    nmem=len(layers[i])
    outfile.write('l {:3} {:3} {:5} {:8.4f} {:8.4f} {:8.4f}\n'.format( i, nmem, nhb[i], nhb[i]/float(nmem), nhac[i]/float(nmem), nhdo[i]/float(nmem) ))
  accfile.close()
  ndxfile.close()
  outfile.close()
