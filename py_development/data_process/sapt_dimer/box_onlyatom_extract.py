...
def main():
  ...
  traj=traj[teq:]
  nstep=traj.n_frames
  print(" total {} frames after initial cutting".format(nstep))
  ...
  snapshot=snapshot.atom_slice([atom.index for atom in topology.atoms if (('D' not in atom.name) and ('M'!=atom.name) and ('OM'!=atom.name) and ('LP' not in atom.name) and ('MO' not in atom.name)) ])
