...
for i in range(n_atoms):
  ...
  neighbor_indices=np.empty(n_atoms,dtype=int)
  for j in range(n_atoms):
    if (i == j):
      continue
    else:
  ...
  centered_sphere_points = np.empty((n_sphere_points,3))
  for j in range(n_sphere_points):
    centered_sphere_points[j] = xyzlist[i] + atom_radius_i*sphere_points[j]
  ...
  for j in range(n_sphere_points): 
    ...
    for k in range(k_closest_neighbor, n_neighbor_indices + k_closest_neighbor):
      ...
      r_jk = np.array(r_j-xyzlist[index],dtype=np.float32)
    ...
    if (is_accessible):
      #if accessible, print atom coord.
      print('X {:8.3f} {:8.3f} {:8.3f}'.format(r_j[0],r_j[1],r_j[2]))
