...
def main():
  ...
  #big loop of sliced topology. inner loop:snapshot loop.
  h0ht,h0htcount,ct=numpy.zeros(int(nstep/2),dtype=float),numpy.zeros(int(nstep/2)), \
  numpy.zeros(int(nstep/2),dtype=float) # autocorrelation function array
  have=0
  for itopb in range(ntopb):
    topstart,topend=itopb*topbl,(itopb+1)*topbl
    if topend>n_angles_full:
      topend=n_angles_full
    partlist_angles=fulllist_angles[topstart:topend]
    hmatrix=numpy.zeros((topend-topstart,nstep),dtype=int)
    #finish constructing h-matrix for the partial topology, through entire trajectory.
    for i_frame in range(nstep):
      traj1=traj[i_frame]
      dist1=(mdtraj.compute_distances(traj1,partlist_angles[:,[1,2]])).flatten()
      angl1=(mdtraj.compute_angles(traj1,partlist_angles)).flatten()
      #before processing angl1 values, need to recalc NaN values
      for nan_item in numpy.argwhere(numpy.isnan(angl1)).reshape(-1):
        i_angle = nan_item%len(angl1)
        i_abc = partlist_angles[i_angle]
      ...
      #for x in index_thresda:
      #  hmatrix[x][i_frame]=1
      hmatrix[index_thresda[:],i_frame]=1

    #partial hmatrix construction is complete. calculate correlation functions and ens average.
    for row in hmatrix:
      have+=numpy.sum(row) #collection for <h> and <h2>
      h0ht_one,h0htcount_one=autocorr_simple(row)
      h0ht+=h0ht_one
      h0htcount+=h0htcount_one

    elapsed=timeit.default_timer() - start_time
    print('topology block# {} time {}'.format(itopb,elapsed))

  unnormh0ht=numpy.copy(h0ht)
  h0ht=numpy.divide(h0ht,h0htcount)
  unnormhave=have
  have=float(have)/(nstep*n_angles_full)
  ct=(h0ht-have*have)/(have-have*have)
  print("<h> and <h2> : {} ".format(have))
  #printing section
  for i in range(len(ct)):
    outfile.write('{:11.6f} {:11.6f} {:11.6f} {:11.6f} {:11.6f} {:11.6f}\n'.format(i*tstep,ct[i],\
    unnormh0ht[i],h0htcount[i],unnormhave,nstep*n_angles_full)) 
