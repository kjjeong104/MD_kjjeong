...
def main():
  ...
  thbave_list=numpy.array([x*10.0 for x in range(1,11)]) #short-time n_HB average candidates
  nhbscut_list=numpy.array([x*0.1 for x in range(5,16)]) #Desired short-time n_HB cutoff for solid identification
  ...
  for thbave in thbave_list: #determined thbave first.
    ...
    if nthbave==5:
      nbin=20
    hist_stave_nhb,bin_edges=numpy.histogram(stave_nhb,bins=nbin,range=[0.0,4.0],density=True)
