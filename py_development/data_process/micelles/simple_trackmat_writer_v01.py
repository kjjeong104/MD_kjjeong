...
# micelle kinetics analysis program supporter
# input file : merged lattice point assignment summary file for entire time period
#now should be able to count even if clustering data has dislocation/fission.
#policy for fission/fusion : just "uncounting" the broken/coalesced micelles
#write tracker matrix file. apply tracker for every consecutive snapshots, to prevent
#wrong assignment.
...
mthres=10 #threshold aggregation number of an independent micelle:6, for tracker(fis/fus), apply 10
...
def main():
  ...
  for i in range(1,nstep):
    ...
    if(i%200==0):
      elapsed=timeit.default_timer() - start_time
      print('tracker matrix writing step {}, time {:11.4f}'.format(i,elapsed))
  elapsed=timeit.default_timer() - start_time
