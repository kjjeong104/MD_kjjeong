...
thr=float(input("threshold for micelle detection by tail position? recomm:0.85\n"))
...
print('finished entry reading {}'.format(elapsed))
...
def morphol(list,box,tempcrd): 
  ...
  for i in range(len(tends)): ###v051 performance upgrade attempt
    for j in range(i,len(tends)):
      for k in range(3):
        if (tempcrd[tends[j]][k]-tempcrd[tends[i]][k])>(box[k]/2.0): #if j is cut by (1b)edge
          if [entry[tends[j]][0],k] not in moveends:
            moveends.append([entry[tends[j]][0],k])
        elif (tempcrd[tends[i]][k]-tempcrd[tends[j]][k])>(box[k]/2.0): #if i is cut by (1b)edge
          if [entry[tends[i]][0],k] not in moveends:
            moveends.append([entry[tends[i]][0],k])
  ...
  elapsed=timeit.default_timer() - start_time
  print('micelle coord adjustment complete, time {}'.format(elapsed))  
...
##part 3: asphericity (using only oxygens. So, calculate oxygens' COM.)
 ...
 #or T_mn = 1/N sum(i) r(i)_m * r(i)_n when origin is set to COM
  ...
  #sv=numpy.sqrt(sv)
  #semiaxis vector (ref: J. Chem. Phys., Vol. 101, No. 10, 15 November 1994)
...
def main():
  ...
  for line in trjfile:
    ...
    if lindex>=7+totnatom: #conclusion for 1 step, and initialization(a big process!)
      totmic,already=[],[]
      elapsed=timeit.default_timer() - start_time
      print('step {} crd reading complete, time {}'.format(sindex,elapsed))
      ...
      for i in range(nsurf):#If not contained: starting a new micelle
        ...
        if lookup2d(i,totmic)==False:
          onemic.append(i)
          search=1
        #print('micelle creation checkpoint1, time {}'.format(timeit.default_timer()-start_time)) ## fast
        while search==1:#starting of repetitive searching algorithm
          search=0
          for k in onemic:#stops when increase of number in onemic stopped
            if k not in already: ###v051 performance upgrade attempt
              for j in range(nsurf):
                if j!=k and lookup2d(j,totmic)==False and j not in onemic:
                  if(distdet(totcrd[tendlist[k]],totcrd[tendlist[j]],thr,box)): #detection
                    onemic.append(j)
                    search=1
              already.append(k)
            #print('micelle creation checkpoint2, time {}'.format(timeit.default_timer()-start_time))
        if len(onemic)!=0:
          onemic.sort()
        ...
        elapsed=timeit.default_timer() - start_time
        print('micelle creation, time {}'.format(elapsed))
      
      elapsed=timeit.default_timer() - start_time
      print('step {} micelle creation complete, time {}'.format(sindex,elapsed))
      ...
      elapsed=timeit.default_timer() - start_time
      print('step {} micelle refinement complete, time {}'.format(sindex,elapsed))

      #com reduction, morphology calculation 
      coms,dias,asps,sv2a,sv2b,sv2c=numpy.empty((0,3),float),numpy.array([]),numpy.array([]),numpy.array([]),numpy.array([]),numpy.array([])
...
format(micindex,len(row),dias[micindex-1],round(asps[micindex-1],4),sv2a[micindex-1],sv2b[micindex-1],sv2c[micindex-1],row))
