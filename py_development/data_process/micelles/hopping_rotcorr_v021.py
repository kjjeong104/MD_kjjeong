...
# v02 : also calculate correlation function without doing initial time averaging.
# not a standard method, but inevitable to inspect.
# v021 : modified code for head/tail only system (C1P1 system)
...
def rot_acf_lmol(trimtraj,i1,i2): #calculator for rotational acf of a linear molecule
  ...
  rot_acf,itrot_acf=numpy.zeros(nsteptau),numpy.zeros(nsteptau)
  ...
  for j in range(nsteptau):
    ...
    for i in range(nstep-j):
      ...
      if i==0:
        itrot_acf[j]=numpy.dot(v0,vt)
      rot_acf[j]+=numpy.dot(v0,vt)
      count+=1
    rot_acf[j]/=count #normalize 

  return rot_acf,itrot_acf
...
def main():
  ...
  i1,i2=0,1 #internal atom indices in a surfactant molecule which directs tail end, headgroup P
...
## when number of frames are not big, then we don't have to divide tau interval to do HT computing.
  ...
  lindex,cfcount=0,0
  tot_rtcf,tot_itrtcf=numpy.zeros(nsteptau),numpy.zeros(nsteptau)
  while True:
    ...
    if info[0]==True: #decided to include that hopping event in calculation
      ...
      onemol_rtcf,onemol_itrtcf=rot_acf_lmol(trimtraj,i1,i2)
      tot_rtcf+=onemol_rtcf
      tot_itrtcf+=onemol_itrtcf
      cfcount+=1
    if lindex%1000==0:
      elapsed=timeit.default_timer() - start_time
  ...
  for i in range(nsteptau):
    tau=i*tstep
    outfile.write('{:9.4f} {:11.6f} {:11.6f}\n'.format(tau,tot_rtcf[i],tot_itrtcf[i]))
