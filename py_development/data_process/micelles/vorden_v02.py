...
# v02 : semiaxis ratio calc, display voronoi volume as well
...
def main():
  ...
  for line1 in incfile:
    if line1[0:3]=='mic': #label line
      ...
      split=line1.split()
      outfile.write('mic# agN    Rg       Asp     sla/c    slb/c    slc/c \
   waterd    TMAd     massd   chgd   vorvol   step {} numofmcls {}\n'.format(split[12],split[14])) 
      ...
      lc=numpy.sqrt(counts[6]) #length of semiaxis c
      for i in range(7,11):
        counts[i]/=float(split2[1]) #dividing 'counts' by voronoi polyhedron volume
      for i in range(4,7):
        counts[i]=numpy.sqrt(counts[i])/lc #take semiaxis ratio
      outfile.write('{:3} {:3} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f}\n'\
.format(int(split1[0]),int(split1[1]),counts[2],counts[3],counts[4],counts[5],counts[6],counts[7],counts[8],counts[9],counts[10],float(split2[1])))
