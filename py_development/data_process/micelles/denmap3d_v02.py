...
def main():
  ...
  surfflag=input("Would you like to count only surfactant mass? yes or no\n")
    ...
    for i in range(nbin[0]):
      ...
      for j in range(nbin[1]):
        for k in range(nbin[2]):
          denfile.write('{:8.5f} {:8.5f} {:8.5f} {:8.5f} {:8.5f}\n'.format(mesh[0]*i,mesh[1]*j,mesh[2]*k,dbin3d[i][j][k],cbin3d[i][j][k]))
        denfile.write(' \n')
      denfile.write(' \n')
