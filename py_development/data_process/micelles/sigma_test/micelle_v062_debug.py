...
# v061: binary data compatibility, v062 merge site-assignment feature
# also reconsideration on micelle judgement threshold decision
# actual test looks bugged : #of micelles, semiaxis length bugged
...
def main():
  ...
  sigfile = open(sys.argv[3],'r')
  outfile = open(sys.argv[4],'w') #output file for result summary
  comfile = open(sys.argv[5],'w') #output file for reduced expression of micelle
  ...
  initt=float(input("Initial time of this trajectory (in ns)? ex) 500.0 \n"))
  ...
  #construct ideal sigma phase lattice points info array (or A15)
  ...
  for sindex in range(nstep):
    ...
    for line in cluslist:
      if line[2]<=dthres: efcllist=numpy.vstack((efcllist,line)) 
    efcllist=efcllist[::-1] #flip matrix
    nlrow=efcllist.shape[0]
    #for bugline in efcllist: print('{:5} {:5} {:8.4f} {:5}'.format(int(bugline[0]),int(bugline[1]),bugline[2],int(bugline[3])))
    ...
    for row in totmic: #tailend member list of 1 micelle
      ...
      for i in mic_atoms_list:
        mic_atoms_crd=numpy.vstack( (mic_atoms_crd,onestep_crd[int(i)]) )
    ...
    #After finishing clustering for 1 step: site number assignment
    tns=initt+ssize*sindex #current time in ns
    oldcoms,oldrgs,oldasps,oldsv2a,oldsv2b,oldsv2c=\
    numpy.copy(coms),numpy.copy(rgs),numpy.copy(asps),numpy.copy(sv2a),numpy.copy(sv2b),numpy.copy(sv2c)
    oldtotmic=list(totmic)
    if coms.shape[0]==totlatp: # number of micelles coincide with lattp
      oindex=0
      lcan,distlog=numpy.zeros(totlatp),numpy.zeros(totlatp) #lattice assignment candidate
      reanum,reaslist,unaslist=[],[],[x for x in range(1,totlatp+1)]
      for row in oldcoms:
        linfo=latticeassign(row,box,sigcrd)
        if linfo[0] in lcan and linfo[0] not in reanum: reanum.append(linfo[0])
        #unaslist.remove(linfo[0])
        lcan[oindex],distlog[oindex]=linfo[0],linfo[1]
        oindex+=1
      if len(reanum)==0: #only if there's no dislocation, try site assignment
       for i in range(totlatp):
         j=lcan[i]-1
         coms[j],rgs[j],asps[j],sv2a[j],sv2b[j],sv2c[j]= \
         oldcoms[i],oldrgs[i],oldasps[i],oldsv2a[i],oldsv2b[i],oldsv2c[i]
         totmic[j]=oldtotmic[i]
    ...
    for row in totmic:
      ...
      rowstr=rowstr.replace("]",",")
      outstr='lattp# '+"{:2} ".format(micindex)+"{:8.3f} ns ".format(tns)+\
      "N= {:3} r= {:8.4f} A= {:8.4f} sv2abc= {:8.4f} {:8.4f} {:8.4f} members: {}\n".\
      format(len(row),rgs[i],round(asps[i],4),sv2a[i],sv2b[i],sv2c[i],rowstr)
      outfile.write(outstr)
    #outfile.write('{} Unaggregated molecules: {}\n'.format(unagg,hom))

    #com output writing section
    outindex=0
    comfile.write('Centers of micelles: step {} t= {:8.3f} ns\n'.format(sindex,tns))
    comfile.write(' {}\n'.format(micindex))
    for row in coms:
      outindex+=1
