...
# Voronoi polyhedron counterion/water inclusion counting program (for a15 phase)
...
# -> classify each MeN, H2O into one of 8 polyhedra, according to closest distance sorting
...
##CAUTION : COMfile should be indexed according to a15 phase lattice site indices
...
def main():
  ...
  waterc,tmac,massc,chgc=numpy.zeros(8,float),numpy.zeros(8,float),numpy.zeros(8,float),numpy.zeros(8,float) 
  for line in comfile:
    if lindex==0:
      split=line.split()
      grostep=int(split[3])
